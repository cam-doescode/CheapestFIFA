...
export function MultiSelect({ label, options, selected, onChange }: MultiSelectProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-56 max-h-64 overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg">
          {selected.length > 0 && (
            <button
              type="button"
              onClick={() => onChange([])}
